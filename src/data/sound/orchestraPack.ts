...
export const ORCHESTRA_SOUND_PACK_ID = "orchestra";

const urls = import.meta.glob(
  "../../assets/audio/packs/orchestra/*.{mp3,wav,ogg}",
  {
    eager: true,
    query: "?url",
    import: "default",
...
const getVariants = (baseName: string): string[][] => {
  ...
  let i = 1;
  if (urlByBaseName.has(baseName)) variants.push(urlByBaseName.get(baseName)!);
...
sfx.combo_milestone = getVariants("combo_milestone");
sfx.game_over = urlByBaseName.has("game_over")
  ? [urlByBaseName.get("game_over")!]
  : sfx.game_over;

export const orchestraSoundPack: SoundPack = {
  ...defaultSoundPack,
  id: ORCHESTRA_SOUND_PACK_ID,
  name: "Orchestra",
  sfx,
